...
const Overlay = ({ merchant, id, onClick, onSuccess, type }) => {
  ...
  const { postData, error, response, loading } = usePutRequest(
    endPoints.chargeBack.acceptFightChargeBack + `${id}` + "/" + `${type}`
  );
...
const CloseChargeBack = ({ merchant, id, onClick, onSuccess, type }) => {
  return (
    <Fragment>
      {createPortal(
...
export default CloseChargeBack;
