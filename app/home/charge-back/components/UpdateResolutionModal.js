...
const Overlay = ({ merchant, id, onClick, onSuccess, type }) => {
  ...
  const { putData, error, response, loading } = usePutRequest(
    endPoints.chargeBack.updateResolution + `${id}` + "/" + `${type}`
  );
    ...
    <div className="overlay w-30">
      ...
      >
        <div className="col-12 mb-2">
          <Label htmlFor="resolution" label="Select Resolution" />
          <select
            name="resolution"
            id="resolution"
...
const UpdateResolution = ({ merchant, id, onClick, onSuccess, type }) => {
  return (
    <Fragment>
      {createPortal(
