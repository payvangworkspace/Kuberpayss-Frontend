...
export default function WaterForm({
  ...
  const locations = [{ id: "Jinja", name: "Jinja" }];
  const [allLocations, setAllLocations] = useState({
    id: "",
    name: "Select Location",
  });
  const handleSelectedLocation = (id, name) => {
    setAllLocations({ id, name });
    setFormData((prev) => ({
      ...prev,
      location: name,
    }));
  ...
  useEffect(() => {
    if (response?.data?.status === "success" && !error) {
      ...
      setAllLocations({ id: "", name: "Select Location" });
  ...
  return (
    <>
      ...
      <div className="row">
        ...
        <div className="col-md-6 col-sm-12 mb-2">
          <Label htmlFor="location" label="location" required={true} />
          <Dropdown
            initialLabel="Select Location"
            selectedValue={allLocations}
            options={locations}
