  ...
  useEffect(() => {
    if (data) {
      ...
      if (chartData?.options) {
        chartData.options.colors = ["#6366f1", "#ef4444", "#f59e0b", "#94a3b8"];
      }
      setState(chartData);
    }
  }, [data]);
      ...
      <div className={styles.chartCard}>
        <div className={styles.chartHeader}>
          <div className={styles.chartHeaderLeft}>
            <span className={`${styles.chartHeaderIcon} ${styles.iconPurple}`}>
              <i className="bi bi-currency-dollar" aria-hidden="true" />
            </span>
            <div>
              <h6 className={styles.chartTitle}>Transaction Amount</h6>
              <p className={styles.chartSubtitle}>
                24Hrs Transaction Amount Analysis
              </p>
            </div>
          </div>
